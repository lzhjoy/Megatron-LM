...
# Dir Arguments
DIR=`pwd`
PRETRAINED_CKPT_ROOT_PATH=${PRETRAINED_CKPT_ROOT_PATH:-"/mnt/nanjingcephfs/project_wx-rec-alg-bdc-exp/bwzheng/yulan/hyw/pretrain-linear-moe-/megatron_lm_workspace"}
PRETRAINED_CKPT_ID=${PRETRAINED_CKPT_ID:-"NOT_EXISTS"}
PRETRAINED_CKPT_NAME=${PRETRAINED_CKPT_NAME:-"NOT_EXISTS"}
OUTPUT_CHECKPOINT_PATH=${OUTPUT_CHECKPOINT_PATH:-"/mnt/nanjingcephfs/project_wx-rec-alg-bdc-exp/bwzheng/yulan/hyw/pretrain-linear-moe-/megatron_lm_workspace"}
...
MOE_UPCYCLING_GRANULARITY=${MOE_UPCYCLING_GRANULARITY:-1}
...
if [ -n "$LOAD_FROM_CHECKPOINT" ]; then
    LOAD_CHECKPOINT_PATH="${LOAD_FROM_CHECKPOINT}"
    CONTINUE_TRAIN=${CONTINUE_TRAIN:-'false'}
    echo -e "\033[32mLoad from dense checkpoint '$LOAD_FROM_CHECKPOINT'\033[0m"
elif [ -d "$CHECKPOINT_PATH/latest_checkpointed_iteration.txt" ]; then
...
DATA_PATH_CACHE="/mnt/nanjingcephfs/project_wx-rec-alg-bdc-exp/bwzheng/yulan/hyw/pretrain-linear-moe-dev/cache/datasets/cache"
if [[ ${TOKENIZER_TYPE} == "hf_tokenizer_qwen" ]]; then
    ...
    TOKENIZER_ARGS="--tokenizer-type HuggingFaceTokenizer --tokenizer-model /mnt/nanjingcephfs/project_wx-rec-alg-bdc-exp/bwzheng/yulan/hyw/pretrain-linear-moe/preprocess/modify_tokenizer/YuLan-Mini-Qwen-Template_new_sp"
else
...
if [ "1${MOE_USE_UPCYCLING}" = "1true" ]; then
    EXTRA_ARGS="${EXTRA_ARGS} \
        --moe-use-upcycling
        --moe-upcycling-granularity ${MOE_UPCYCLING_GRANULARITY}
    "
    rm -rf ${CHECKPOINT_PATH}
    echo -e "\033[32mUsing MOE Upcycling, remove checkpoint ${CHECKPOINT_PATH}\033[0m"
fi

pretrain_script=../../pretrain_gpt.py

torchrun ${DISTRIBUTED_ARGS[@]} ${pretrain_script} \
    ${MODEL_ARGS[@]} \
    ${DATA_ARGS[@]} \
    ${MOE_ARGS[@]} \
    ${TRAINING_ARGS[@]} \
