...
def validate_model_config_args_from_heterogeneous_config(args):
    ...
    Returns:
        None
    """
    if (args.heterogeneous_layers_config_path is None
    ...
    if args.heterogeneous_layers_config_encoded_json is None:
        args.heterogeneous_layers_config_encoded_json = Path(
            args.heterogeneous_layers_config_path).read_text()
    ...
    args_to_validate = {
        ...
        True,
        "num_layers":
        hf_config_dict.num_hidden_layers,
        "hidden_size":
        hf_config_dict.hidden_size,
        "num_attention_heads":
        hf_config_dict.num_attention_heads,
        "untie_embeddings_and_output_weights":
        not hf_config_dict.tie_word_embeddings,
        "rotary_base":
        hf_config_dict.rope_theta,
        "rope_scaling_factor":
        hf_config_dict.rope_scaling["factor"],
        "num_query_groups":
        hf_config_dict.num_attention_heads // n_kv_heads_in_group[0],
    }
...
def validate_args(args, defaults={}):
    ...
    if args.attention_backend == AttnBackend.local:
        assert args.spec[
            0] == 'local', '--attention-backend local is only supported with --spec local'
    ...
    use_data_path = (args.data_path is not None) or (args.data_args_path
                                                     is not None)
    if use_data_path:
    ...
    if args.use_torch_fsdp2:
        ...
        if args.fp4_param and not is_te_min_version("2.7.0.dev0"):
            raise ValueError(
                "--fp4-param requires Transformer Engine >= 2.7.0.dev0.")
    ...
    if args.fp4_param and not args.fp4:
        raise ValueError(
            "--fp4-param-gather must be used together with --fp4-format.")
    ...
    if args.moe_grouped_gemm:
        ...
        assert dc[
            0] >= 8, "Unsupported compute capability for GroupedGEMM kernels."
    ...
    if args.use_torch_fsdp2 or args.use_megatron_fsdp and get_device_arch_version(
    ) >= 10:
    ...
    if args.rotary_interleaved and args.use_legacy_models:
        raise RuntimeError(
            '--rotary-interleaved is not supported in legacy models.')
    ...
    if args.num_experts is not None:
        assert (
            args.spec is None or args.spec == [
    ...
    if args.num_experts is not None and args.moe_ffn_hidden_size is None:
        ...
        print(
            "Warning: moe_ffn_hidden_size is not set, using ffn_hidden_size for MoE instead."
        )
    ...
    # MoE router check
    if isinstance(args.moe_router_load_balancing_type, list) and len(
            args.moe_router_load_balancing_type) == 1:
        args.moe_router_load_balancing_type = args.moe_router_load_balancing_type[
            0]
    if isinstance(args.moe_aux_loss_coeff, list) and len(
            args.moe_aux_loss_coeff) == 1:
        args.moe_aux_loss_coeff = args.moe_aux_loss_coeff[0]

    # Distributed checkpointing checks
    if args.use_dist_ckpt and args.use_legacy_models:
        raise RuntimeError(
            '--use-dist-ckpt is not supported in legacy models.')
    ...
    if args.deterministic_mode:
        ...
        all_reduce_choices = [
            "Tree", "Ring", "CollnetDirect", "CollnetChain", "^NVLS"
        ]
    ...
    if (args.use_dist_ckpt and not args.ckpt_fully_parallel_save
            and args.use_distributed_optimizer and args.rank == 0):
        print(
    ...
    if args.replication:
        ...
        print(
            "Warning: --replication-jump was specified despite not using replication. Ignoring."
        )
    ...
    if args.cuda_graph_impl != "none":
        if args.transformer_impl == 'transformer_engine' and not args.te_rng_tracker:
            ...
            warn_rank_0(
                ...
                args.rank)
        assert "expandable_segments:True" not in os.getenv(
...
def core_transformer_config_from_args(args, config_class=None):
    ...
    if hasattr(args,
               "kitchen_config_file") and args.kitchen_config_file is not None:
        kw_args['use_kitchen'] = True
        kw_args['quant_recipe'] = load_quantization_recipe(
            args.kitchen_config_file)
    elif hasattr(args, 'kitchen_recipe_number'
...
def _add_transformer_engine_args(parser):
    ...
    group.add_argument('--fp8-amax-compute-algo',
                       default='most_recent',
    ...
    group.add_argument(
        ...
        dest='fp8_wgrad')
    group.add_argument('--transformer-impl',
...
def _add_inference_args(parser):
    ...
    group.add_argument("--cuda-graph-warmup-steps",
                       type=int,
                       default=3,
    ...
    group.add_argument('--inference-max-batch-size',
                       type=int,
                       default=None,
    ...
    group.add_argument(
        ...
        help=
        'What type of symmetric all reduce to use. The default is none which is no use of symetric memory'
    )
    group.add_argument(
        ...
        default=False,
        help=
        'When using symmeric all reduce kernels this will use regular nccl kernels for prefill. This can be more effecient when prefill is large as the nccl kernels can be more bandwith optimized'
    )
    group.add_argument(
...
def _add_retro_args(parser):
    ...
    group.add_argument('--retro-project-dir',
                       default=None,
    ...
    group.add_argument('--retro-cyclic-train-iters',
                       type=int,
                       default=None,
                       help='Set number of training iterations for cyclic '
                       'Retro training.')
    group.add_argument('--retro-encoder-layers',
                       type=int,
                       default=2,
    ...
    group.add_argument('--retro-encoder-hidden-dropout',
                       type=float,
                       default=0.1,
                       help='Hidden dropout for '
                       'retrieval encoder.')
    group.add_argument('--retro-encoder-attention-dropout',
                       type=float,
                       default=0.1,
                       help='Attention dropout for '
                       'retrieval encoder.')
    group.add_argument("--retro-num-neighbors",
                       type=int,
                       default=2,
                       help='Number of neighbors to retrieve during '
                       'pretraining.')
    group.add_argument("--retro-num-retrieved-chunks",
                       type=int,
                       default=2,
                       help='Number of chunks to retrieve from the retrieval '
                       'database.')
    group.add_argument("--retro-attention-gate",
                       type=float,
                       default=1,
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--encoder-num-layers',
                       type=int,
                       default=None,
                       help='Number of encoder transformer layers.')
    group.add_argument('--decoder-num-layers',
                       type=int,
                       default=None,
    ...
    group.add_argument('--kv-channels',
                       ...
                       'if not provided.')
    group.add_argument('--group-query-attention',
                       action='store_true',
    ...
    group.add_argument('--use-rotary-position-embeddings',
                       action='store_true',
    ...
    group.add_argument('--rotary-interleaved',
                       action='store_true',
    ...
    group.add_argument('--use-rope-scaling',
                       action='store_true',
    ...
    group.add_argument('--normalization',
                       default='LayerNorm',
                       choices=['LayerNorm', 'RMSNorm'],
                       help='Which normalization technique to use.')
    group.add_argument('--norm-epsilon',
                       type=float,
                       default=1e-5,
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        'Use gated linear units and squared relu activation instead of default gelu'
    )
    group.add_argument('--geglu',
                       action='store_true',
    ...
    group.add_argument('--untie-embeddings-and-output-weights',
                       action='store_true',
                       help='Untie embeddings and output weights.')
    group.add_argument('--multi-latent-attention',
                       action='store_true',
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        choices=['cat', 'subtraction', 'addition'],
        help=
        'Whether to use token time-shift before FFN. cat: https://zhuanlan.zhihu.com/p/399480671 '
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        choices=['kv_shifting', 'kv_shifting_one'],
        help=
        'Whether to use token time-shift before RoPE and core attention. kv_shifting: http://arxiv.org/abs/2411.19574 '
    )
    group.add_argument('--attn-output-gate',
                       type=str,
                       default=None,
                       choices=['full', 'lora'],
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        choices=['full', 'full_rope'],
        help='Whether to use PaTH attention. http://arxiv.org/abs/2505.16381')
    group.add_argument(
        '--shortconv-kernel-size',
        type=int,
        default=3,
        help='Kernel size for short convolution in PaTH attention.')
    group.add_argument(
        '--emb-deviation-loss-coeff',
        type=float,
        default=0,
        help=
        'Scaling coefficient for the embedding devication loss. Default 0 means disabled.'
    )
    group.add_argument('--emb-deviation-type',
                       type=str,
                       default=None,
                       choices=['loss', 'square_loss', 'mean'],
...
def _add_straggler_detector_args(parser):
    ...
    group.add_argument('--log-straggler',
                       action='store_true',
...
def _add_inprocess_restart_args(parser):
    ...
    group.add_argument('--inprocess-restart',
                       action='store_true',
                       help='Enables in-process restart.')

    group.add_argument('--inprocess-max-iterations',
                       default=None,
                       type=int,
    ...
    group.add_argument('--inprocess-soft-timeout',
                       default=60,
                       type=float,
                       help='Soft progress timeout (in seconds).')
    group.add_argument('--inprocess-hard-timeout',
                       default=90,
                       type=float,
    ...
    group.add_argument('--inprocess-granularity',
                       default='node',
                       type=str,
                       choices=['node', 'rank'],
...
def _add_one_logger_args(parser):
    ...
    group.add_argument('--one-logger-project',
                       type=str,
                       default='megatron-lm',
    ...
    group.add_argument(
        ...
        '--no-one-logger is set')
    group.add_argument('--one-logger-async',
                       action='store_true',
...
def _add_ft_package_args(parser):
    ...
    group.add_argument('--enable-ft-package',
                       action='store_true',
...
def _add_logging_args(parser):
    ...
    group.add_argument('--log-throughput',
                       action='store_true',
    ...
    group.add_argument(
        ...
        'directory.')
    group.add_argument('--timing-log-level',
                       type=int,
                       default=0,
                       choices=range(0, 3),
    ...
    group.add_argument('--log-energy',
                       action='store_true',
                       help='If set, log energy consumption (in Joules)')
    group.add_argument(
        '--increase-log-level-interval',
        type=int,
        default=1000,
        help=
        'The number of steps in each cycle for increasing the log level to 2.')
    group.add_argument(
        '--increase-log-level-iters',
        type=int,
        default=5,
        help='The number of steps to increase the log level to 2 in each cycle.'
    )
    group.add_argument('--no-barrier-with-level-1-timing',
                       action='store_false',
    ...
    group.add_argument('--timing-log-option',
                       type=str,
                       default='minmax',
    ...
    group.add_argument('--wandb-exp-name',
                       type=str,
                       default='',
                       help='The wandb experiment name.')
    group.add_argument('--wandb-save-dir',
                       type=str,
                       default='',
...
def _add_regularization_args(parser):
    ...
    group.add_argument('--word-embedding-dropout-prob',
                       type=float,
                       default=0.0,
    ...
    group.add_argument('--weight-decay-incr-style',
                       type=str,
                       default='constant',
    ...
    group.add_argument('--muon-momentum',
                       type=float,
                       default=0.95,
    ...
    group.add_argument('--no-muon-nesterov',
                       action='store_false',
                       dest='muon_nesterov',
                       default=True,
...
def _add_rl_args(parser):
    ...
    group.add_argument('--perform-rl-step',
                       action='store_true',
    ...
    group.add_argument('--grpo-prompts-per-step',
                       type=int,
                       default=32,
                       help="Number of GRPO groups (G in the paper).")
    group.add_argument('--grpo-group-size',
                       type=int,
                       default=2,
                       help="Number of samples per a GRPO group.")
    group.add_argument('--grpo-iterations',
                       type=int,
                       default=2,
    ...
    group.add_argument('--grpo-clamp-eps-lower',
                       type=float,
                       default=0.01,
    ...
    group.add_argument('--grpo-kl-beta',
                       type=float,
                       default=0.001,
                       help="KL term weight in the GRPO loss.")
    group.add_argument('--grpo-entropy-term-weight',
                       type=float,
                       default=0.0,
                       help="Entropy term weight in GRPO loss.")
    group.add_argument('--grpo-filter-groups-with-same-reward',
                       action='store_true',
                       help="Filter groups with same reward.")
    group.add_argument('--grpo-default-temperature',
                       type=float,
                       default=1.0,
                       help="Default temperature for model inference.")
    group.add_argument('--grpo-default-top-p',
                       type=float,
                       default=0,
                       help="Default top-p for model inference.")
    group.add_argument('--langrl-inference-server-type',
                       type=str,
                       choices=['inplace_megatron', 'inplace_megatron_chat'],
                       default='inplace_megatron',
                       help="Type of inference server to use.")
    group.add_argument('--langrl-inference-server-conversation-template',
                       type=str,
                       default=None,
                       help="Conversation template, if using a chat server.")
    group.add_argument('--langrl-external-server',
    ...
    group.add_argument(
        ...
        default=None,
        help=
        "If --inference-logprobs-is-correction is on and this coefficient is set, apply truncation for the IS correction at GRPO loss."
    )
    group.add_argument(
    ...
    group.add_argument('--rl-use-sequence-packing',
                       action='store_true',
                       help='Enable sequence packing')
    group.add_argument('--rl-sequence-packing-bin-size',
                       type=int,
                       default=8192,
...
def _add_training_args(parser):
    ...
    group.add_argument('--recompute-activations',
                       action='store_true',
                       help='recompute activation to allow for training '
                       'with larger models, sequences, and batch sizes.')
    group.add_argument('--recompute-granularity',
                       type=str,
                       default=None,
    ...
    group.add_argument('--no-check-for-nan-in-loss-and-grad',
                       action='store_false',
                       help='Check for NaNs in loss and grad',
                       dest='check_for_nan_in_loss_and_grad')
    group.add_argument('--check-for-spiky-loss',
                       action='store_true',
                       help='Check for spiky loss',
                       dest='check_for_spiky_loss')
    group.add_argument('--check-for-large-grads',
                       action='store_true',
    ...
    group.add_argument('--profile-step-start',
                       type=int,
                       default=10,
                       help='Global step to start profiling.')
    group.add_argument('--profile-step-end',
                       type=int,
                       default=12,
                       help='Global step to stop profiling.')
    group.add_argument('--iterations-to-skip',
                       nargs='+',
                       type=int,
                       default=[],
    ...
    group.add_argument('--use-pytorch-profiler',
                       action='store_true',
    ...
    group.add_argument('--record-memory-history',
                       action="store_true",
                       default=False,
    ...
    group.add_argument('--tp-comm-overlap-cfg',
                       type=str,
                       default=None,
                       help='Config file when tp_comm_overlap is enabled.')
    group.add_argument('--disable-tp-comm-overlap-ag',
                       action='store_false',
    ...
    group.add_argument('--empty-unused-memory-level',
                       default=0,
                       type=int,
                       choices=[0, 1, 2],
    ...
    group.add_argument('--exit-signal-handler',
                       action='store_true',
    ...
    group.add_argument('--no-rope-fusion',
                       action='store_false',
    ...
    group.add_argument('--use-flash-attn',
                       action='store_true',
                       help='use FlashAttention implementation of attention. '
                       'https://arxiv.org/abs/2205.14135')
    group.add_argument('--disable-bias-linear',
                       action='store_false',
                       help='Disable bias in the linear layers',
                       dest='add_bias_linear')
    group.add_argument('--add-qkv-bias',
                       action='store_true',
    ...
    group.add_argument('--optimizer-cpu-offload',
                       action='store_true',
                       help='Offload optimizer state to CPU')
    group.add_argument('--optimizer-offload-fraction',
                       type=float,
                       default=1.0,
    ...
    group.add_argument('--no-pin-cpu-grads',
                       action='store_false',
                       dest='pin_cpu_grads',
                       help='Disable pinning of CPU memory for gradients.')
    group.add_argument('--no-pin-cpu-params',
                       action='store_false',
                       dest='pin_cpu_params',
    ...
    group.add_argument(
        ...
        'size is supported.')
    group.add_argument('--sequence-parallel',
                       action='store_true',
    ...
    group.add_argument('--manual-gc-interval',
                       type=int,
                       default=0,
    ...
    group.add_argument(
        ...
        choices=['nccl', 'ucc'],
        help=
        'Select a communicator backend for pipeline parallel communication. '
    ...
    group.add_argument('--freeze-non-mamba',
                       action='store_true',
...
def _add_rerun_machine_args(parser):
    ...
    group.add_argument('--error-injection-rate',
                       type=int,
                       default=0,
...
def _add_initialization_args(parser):
    ...
    group.add_argument('--seed',
                       ...
                       'pytorch, and cuda.')
    group.add_argument('--data-parallel-random-init',
                       action='store_true',
...
def _add_learning_rate_args(parser):
    ...
    group.add_argument('--lr-warmup-samples',
                       ...
                       'learning rate over.')
    group.add_argument('--lr-warmup-init',
                       type=float,
                       default=0.0,
    ...
    group.add_argument('--override-opt_param-scheduler',
    ...
    group.add_argument(
        ...
        default=None,
        help=
        'Minimum value for learning rate for the input and output layer. The scheduler'
        'clip values below this threshold')
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument('--no-save-step-one',
                       action='store_true',
                       default=None,
    ...
    group.add_argument('--load-main-params-from-ckpt',
                       action='store_true',
                       default=None,
    ...
    group.add_argument('--exit-on-missing-checkpoint',
                       action='store_true',
                       help="If '--load' is set, but checkpoint is not found "
                       "(e.g., path typo), then exit instead of random "
                       "initialization.")
    group.add_argument('--use-dist-ckpt',
                       action='store_true',
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        'Determine if the checkpoint format is in legacy or distributed format.'
    ...
    group.add_argument('--ckpt-convert-format',
                       default=None,
                       choices=['torch', 'torch_dist', 'zarr'],
                       help='Checkpoint format for conversion.')
    group.add_argument('--ckpt-convert-save',
                       default=None,
    ...
    group.add_argument('--ckpt-fully-parallel-save',
                       action='store_true',
    ...
    group.add_argument('--ckpt-fully-parallel-load',
                       action='store_true',
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        'Make optimizer distributed checkpoint fully reshardable (TP/PP/EP/DP)'
...
def _add_mixed_precision_args(parser):
    ...
    group.add_argument('--grad-reduce-in-bf16',
                       action='store_true',
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--distributed-timeout-minutes',
                       type=int,
                       default=10,
    ...
    group.add_argument('--overlap-grad-reduce',
                       action='store_true',
                       default=False,
    ...
    group.add_argument('--ddp-bucket-size',
                       type=int,
                       default=None,
    ...
    group.add_argument('--use-distributed-optimizer',
                       action='store_true',
    ...
    group.add_argument(
        ...
        default=False,
        help=
        'Disable symmetric (window) registration for NCCL userbuffer registration.'
        'This option will force to use conventional (local) userbuffer registration when use-nccl-ub is set.'
    )
    group.add_argument('--use-sharp',
                       action='store_true',
    ...
    group.add_argument('--use-megatron-fsdp',
                       action='store_true',
    ...
    group.add_argument(
        ...
        default=None,
        help=
        'Specifies the number of elements to communicate at once during FSDP (Fully Sharded Data Parallel) operations. '
...
def _add_validation_args(parser):
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        'If set, multiple datasets listed in the validation split are evaluated independently with a separate loss for each dataset in the list. This argument requires that no weights are included in the list'
    )
...
def _add_tokenizer_args(parser):
    ...
    group.add_argument('--tokenizer-type',
                       ...
                       choices=[
                           ...
                           'Llama2Tokenizer', 'TikTokenizer',
                           'MultimodalTokenizer', 'NullTokenizer',
    ...
    group.add_argument('--tokenizer-metadata',
                       type=str,
                       default=None,
                       help='Path to tokenizer metadata in json format.')
    group.add_argument('--tiktoken-pattern',
                       type=str,
                       default=None,
                       help='Which tiktoken pattern to use. Options: [v1, v2]')
    group.add_argument('--tiktoken-num-special-tokens',
                       type=int,
                       default=1000,
    ...
    group.add_argument('--legacy-tokenizer',
                       action='store_true',
                       default=False,
...
def _add_data_args(parser):
    ...
    group.add_argument(
        ...
        default=None,
        help=
        'The weight and prefix list for a set of train, validation, and test'
    ...
    group.add_argument('--data-cache-path',
                       default=None,
                       help='Path to a directory to hold cached index files.')
    group.add_argument('--no-mmap-bin-files',
                       action='store_false',
    ...
    group.add_argument(
        ...
        type=str,
        default='random',
        choices=['random', 'ffd', 'bfd'],
        help=
        'Algorithm to pack the documents. Currently supports random shuffle '
        'and two fewer truncation algorithms (first-fit decreasing and best-fit decreasing).'
    )
    return parser
...
def _add_vision_args(parser):
    ...
    group.add_argument('--img-h',
                       type=int,
                       default=224,
                       help='Image height for vision classification task')
    group.add_argument('--img-w',
                       type=int,
                       default=224,
                       help='Image height for vision classification task')
    ...
    group.add_argument('--classes-fraction',
                       type=float,
                       default=1.0,
                       help='training with fraction of classes.')
    group.add_argument('--data-per-class-fraction',
                       type=float,
                       default=1.0,
                       help='training with fraction of data per class.')
    group.add_argument('--no-data-sharding',
                       action='store_false',
    ...
    group.add_argument('--vision-pretraining',
                       action='store_true',
                       help='flag to indicate vision pretraining')
    group.add_argument('--vision-pretraining-type',
                       type=str,
                       default='classify',
                       choices=['classify', 'inpaint', 'dino'],
                       help='pretraining objectives')
    group.add_argument('--vision-backbone-type',
                       type=str,
                       default='vit',
                       choices=['vit', 'mit', 'swin'],
                       help='backbone types types')
    group.add_argument('--swin-backbone-type',
                       type=str,
                       default='tiny',
                       choices=['tiny', 'base', 'h3'],
                       help='pretraining objectives')
    # inpainting arguments
    group.add_argument('--mask-type',
                       type=str,
                       default='random',
                       choices=['random', 'row'],
                       help='mask types')
    group.add_argument('--mask-factor',
                       type=float,
                       default=1.0,
                       help='mask size scaling parameter')

    # dino arguments
    group.add_argument('--iter-per-epoch',
                       type=int,
                       default=1250,
                       help='iterations per epoch')
    group.add_argument('--dino-local-img-size',
                       type=int,
                       default=96,
                       help='Image size for vision classification task')
    group.add_argument('--dino-local-crops-number',
                       type=int,
                       default=10,
                       help='Number of local crops')
    group.add_argument('--dino-head-hidden-size',
                       type=int,
                       default=2048,
                       help='Hidden dimension size in dino head')
    group.add_argument('--dino-bottleneck-size',
                       type=int,
                       default=256,
                       help='Bottle neck dimension in dino head ')
    group.add_argument('--dino-freeze-last-layer',
                       type=float,
                       default=1,
                       help='Freezing last layer weights')
    group.add_argument('--dino-norm-last-layer',
                       action='store_true',
                       help='Disable Norm in last layer.')
    group.add_argument('--dino-warmup-teacher-temp',
                       type=float,
                       default=0.04,
                       help='warump teacher temperature')
    group.add_argument('--dino-teacher-temp',
                       type=float,
                       default=0.07,
                       help='teacher temperature')
    group.add_argument('--dino-warmup-teacher-temp-epochs',
                       type=int,
                       default=30,
...
def _add_moe_args(parser):
    ...
    # General arguments
    group.add_argument('--expert-model-parallel-size',
                       type=int,
                       default=1,
    ...
    group.add_argument(
        ...
        default=None,
        help=
        'Degree of expert model parallelism. Default is None, which will be set to the value of --tensor-model-paralle-size.'
    )
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        'When there are multiple experts per rank, launch multiple local GEMM kernels in multiple streams to improve the utilization and performance with GroupedLinear in TransformerEngine.'
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        'Load a checkpoint of a dense model, convert it into an MoE model, and save the converted model to the path specified by --save. '
    ...
    group.add_argument(
        ...
        default='aux_loss',
        help=
        'Determines the load balancing strategy for the router. "aux_loss" corresponds to the load balancing loss used in GShard and SwitchTransformer; "seq_aux_loss" corresponds to the load balancing loss used in DeepSeekV2, which computes the loss for each individual sample; "sinkhorn" corresponds to the balancing algorithm used in S-BASE, and "none" implies no load balancing. The default is "aux_loss".'
    )
    group.add_argument(
        '--moe-router-dtype',
        type=str,
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        'Enable fusion for MoE TopK routing and aux-loss computation. This is only supported in TransformerEngine 2.7.0 and above.'
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        default=None,
        help=
        'Scaling factor for routing score in top-k selection, only works when --moe-router-pre-softmax enabled. Defaults to None, which means no scaling.'
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        default=1e-3,
        help=
        'Expert bias update rate in the aux-loss-free load balancing strategy. '
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        '[Experimental] Force override routing to balance token distribution using random logits for MoE routers, supporting naive top-k and group-limited top-k. This experimental feature is for benchmarking purposes only!'
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        default='allgather',
        help=
        "The type of token dispatcher to use. The default is 'allgather'. Options are 'allgather', 'alltoall'. We recommend using 'alltoall' when applying expert parallelism. For more information, please refer to the documentation in core/moe/README."
    )
    group.add_argument(
        '--moe-enable-deepep',
        action='store_true',
        help=
        '[Experimental] Enable DeepSeek/DeepEP for efficient token dispatching and combine in MoE models. Only works with flex token dispatcher by setting --moe-token-dispatcher-type=flex.'
    )
    group.add_argument('--moe-deepep-num-sms',
                       type=int,
                       default=20,
    ...
    group.add_argument(
        ...
        action='store_true',
        help=
        'Pads the input for each expert to match the expert capacity length, effective only after the --moe-expert-capacity-factor is set.'
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        default=1,
        help=
        'This param sepecifics how many times smaller is the expert hidden size compared with the original dense FFN hidden size. '
...
def _add_mla_args(parser):
    ...
    group.add_argument('--q-lora-rank',
                       type=int,
                       default=None,
...
def _add_heterogeneous_args(parser):
    ...
    group.add_argument(
        ...
        default=None,
        help=
        'This is encoded json string of the heterogeneous model configuration. Used to keep the content '
...
def _add_experimental_args(parser):
    ...
    group.add_argument('--enable-experimental',
                       action='store_true',
    ...
    group.add_argument(
        ...
        'range [0.0, 1.0].')
    group.add_argument('--hybrid-mlp-ratio',
                       type=float,
                       default=0.0,
                       help='Ratio of mlp layers to total layers, in the '
                       'range [0.0, 1.0].')
    group.add_argument('--hybrid-override-pattern',
                       type=str,
                       default=None,
...
def _add_kitchen_quantization_arguments(parser: argparse.ArgumentParser):
    ...
    if have_kitchen:
        ...
        recipe_or_config_group = group.add_mutually_exclusive_group(
            required=False)
...
def _add_sft_args(parser):
    ...
    group.add_argument('--sft',
                       action="store_true",
                       help='Megatron SFT training')
    group.add_argument('--sft-tokenizer-prompt-format',
                       type=str,
